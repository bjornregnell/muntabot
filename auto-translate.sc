...
// concept-headings-GENERATED.scala) and writes TWO English artifacts
// (data-en.scala, concept-headings-En-GENERATED.scala).
//
// It does NOT write headingTranslateSvEn any more: introprog owns that map and
// `sbt syncMuntabot` delivers it as heading-translate-GENERATED.scala. See the
// note above genConceptHeadingsEn.
//
// Determinism & safety:
...
def bootstrap(): Unit =
  // introprog's authoritative sv->en heading map (FindHeadings.writeTranslateMap): already a map,
  // and since 2026-08-22 it is the ONE writer of those pairs, so seeding from it is free accuracy.
  val hEn = ScalaDir / "heading-translate-GENERATED.scala"
...
// headingTranslateSvEn IS NO LONGER GENERATED HERE (2026-08-22).
// It used to be translated by the local model from the #nameddest anchors in data-sv.scala,
// while introprog independently produced English text for the very same headings from its
// reviewed cache + Overrides. Two writers, no shared source, free to disagree -- and they did:
// of the 157 headings both sides translated, 85 differed. introprog now emits the map by
// joining compendium.toc and compendium-en.toc on section number (no model at all), and
// `sbt syncMuntabot` ships it here as heading-translate-GENERATED.scala.
// Do not re-add a writer for that file. To fix a label, fix the English compendium.
