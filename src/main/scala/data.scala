...
lazy val terms = Seq[(Week, Concepts | Contrasts | Code)](
  ...
  Week(3) -> Concepts("anonym funktion", "predikat", "äkta funktion", "aktiveringspost", "anropsstacken", "default-argument",  "returtyp", "namngivna argument", "lokal funktion", "funktion som äkta värde"),
  Week(3) -> Contrasts("överlagring" -> "default-argument", "namngivna argument" -> "default-argument"),
  Week(3) -> Code("en anonym funktion Int => Int som ger kvadraten och som appliceras på alla element i en heltalssekvens"),
  Week(3) -> Code("en definition av en oäkta funktion som returnerar slumpmässiga jämna heltal mellan n och m"),
  ...
  Week(7) -> Code("en funktion copy som kopierar en Array med heltal element för element till en ny Array,\nanvänd en while-sats i implementationen"),
  Week(7) -> Code("en funktion insert som givet en Array med heltal stoppar in ett element på en viss plats,\nanvänd en while-sats i implementationen"),
  Week(7) -> Code("en funktion som avgör om två strängar är lika,\nanvänd en while-sats i implementationen"),
