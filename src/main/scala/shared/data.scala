...
lazy val terms = Seq[(Week, Concepts | Contrasts | Code)](
  ...
  Week(7) -> Code(
    "funktion copy som kopierar en Array element för element till en ny Array,\nanvänd en while-sats i implementationen"
  ),
  Week(7) -> Code(
    "funktion insert som givet en Array stoppar in ett element på en viss plats,\nanvänd en while-sats i implementationen"
  ),
  Week(7) -> Code(
    "funktion som avgör om två strängar är lika,\nanvänd en while-sats i implementationen"
  ),
  ...
  Week(9) -> Code(
    "en funktion som läser in rader med efternamn, förnamn från en textfil\noch skapar en sekvens av instanser av case-klassen Person"
  ),
  Week(10) -> Concepts(
    "arv",
    "subtypspolymorfism",
